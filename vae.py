...
class VAE(tf.keras.Model):
    def __init__(self, args):
        ...
        z_log_variance = tf.keras.layers.Dense(args.z_dim)(x)

        self.encoder = tf.keras.Model(inputs=input, outputs=[z_mean, z_log_variance])
    ...
    def train_step(self, images):
        with tf.GradientTape() as tape:
            ...
            distribution = tfp.distributions.Normal(loc=z_mean, scale=tf.math.exp(z_log_variance / 2))
