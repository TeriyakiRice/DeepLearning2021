...
class Network(tf.keras.Model):
    def __init__(self, args, train):
        ...
        x = tf.keras.layers.Embedding(input_dim=train.forms.word_mapping.vocab_size(), output_dim=args.we_dim)(x)
        ...
        x = tf.keras.layers.Bidirectional(cells_dict[args.rnn_cell], merge_mode='sum')(x)
        ...
        layer_to_be_distributed = tf.keras.layers.Dense(activation=None,
                                                        units=train.tags.word_mapping.vocab_size()
                                                        )
        predictions = tf.keras.layers.TimeDistributed(layer_to_be_distributed)(x)
        ...
        unique_tags = train.tags.word_mapping.vocab_size()
        # zeros = tf.Variable(tf.zeros_initializer()(shape=[unique_tags,unique_tags],dtype=tf.float32),trainable=True)
        self._crf_weights = self.add_weight(shape=[unique_tags, unique_tags], trainable=True, dtype=tf.float32,
    ...
    def crf_loss(self, gold_labels, logits):
        ...
        gold_labels = gold_labels.to_tensor()
        logits = logits.to_tensor()
        log_likelihood, _ = tfa.text.crf_log_likelihood(sequence_lengths=sequence_length,
                                                        transition_params=self._crf_weights,
                                                        tag_indices=gold_labels,
                                                        inputs=logits)
    ...
    def crf_decode(self, logits):
        ...
        logits = logits.to_tensor()
        predictions, _ = \
            tfa.text.crf_decode(potentials=logits, transition_params=self._crf_weights,
                                sequence_length=sequence_length)
        predictions = tf.RaggedTensor.from_tensor(predictions, lengths=sequence_length)
    ...
    class SpanLabelingF1Metric(tf.metrics.Metric):
        ...
        def classify_spans(self, y_true, y_pred, sentence_limits):
            ...
            return np.array([len(spans_true & spans_pred), len(spans_pred - spans_true), len(spans_true - spans_pred)],
                            np.int64)
        ...
        def update_state(self, y_true, y_pred, sample_weight=None):
            ...
            counts = tf.numpy_function(self.classify_spans, (y_true.values, y_pred.values, y_true.row_limits()),
                                       tf.int64)
