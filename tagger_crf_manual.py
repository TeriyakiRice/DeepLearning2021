...
class Network(tf.keras.Model):
    def __init__(self, args, train):
        ...
        if args.rnn_cell == "LSTM":
            sequenceLayer = tf.keras.layers.LSTM(args.rnn_cell_dim, return_sequences=True)
        elif args.rnn_cell == "GRU":
            sequenceLayer = tf.keras.layers.GRU(args.rnn_cell_dim, return_sequences=True)

        xNotRagged = tf.keras.layers.Bidirectional(sequenceLayer, merge_mode='sum')(x.to_tensor(), mask=tf.sequence_mask(x.row_lengths()))
        x = tf.RaggedTensor.from_tensor(xNotRagged, x.row_lengths())
        ...
        predictions = tf.keras.layers.TimeDistributed(tf.keras.layers.Dense(numberOfUniqueTags, activation=None))(x)
        ...
        self._crf_weights = self.add_weight('weights', shape=[numberOfUniqueTags, numberOfUniqueTags], initializer=tf.initializers.Zeros(), dtype=tf.float32, trainable=True)
    ...
    def crf_loss(self, gold_labels, logits):
        ...
        firstSum = tf.gather(logits, gold_labels, batch_dims=2)
        firstSum = tf.reduce_sum(firstSum, 1)
        
        indices = tf.stack([gold_labels[:, :-1], gold_labels[:, 1:]], axis=2)

        A = self._crf_weights
        secondSum = tf.gather_nd(A, indices)
        secondSum = tf.reduce_sum(secondSum, 1)

        expandedA = tf.expand_dims(A, 0)
        def stepFunction(ans, x):
            ans = tf.expand_dims(ans, 2)
            return x + tf.reduce_logsumexp(ans + expandedA, 1)

        alpha_0 = tf.squeeze(logits[:, 0:1, :], 1)
        logitsTransposed_t = tf.transpose(logits[:, 1:, :].to_tensor(), [1, 0, 2])

        alphaTransposed = tf.scan(stepFunction, logitsTransposed_t, alpha_0)
        alpha_t = tf.transpose(alphaTransposed, [1, 0, 2])
        alpha = tf.concat([tf.expand_dims(alpha_0, 1), alpha_t], 1)

        logSumExp = tf.reduce_logsumexp(tf.gather(alpha, gold_labels.row_lengths() - 1, batch_dims=1), 1)

        log_likelihoods = firstSum + secondSum - logSumExp
        return tf.abs(tf.reduce_mean(log_likelihoods))
