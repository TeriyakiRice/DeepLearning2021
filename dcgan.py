...
def dense_batchnorm_relu(units):
    return batchnorm_relu(tf.keras.layers.Dense(units, use_bias=False))
...
class GAN(tf.keras.Model):
    def __init__(self, args):
        ...
        # - applies transposed convolution with 1 filter, kernel size 5,
        #   stride 2, same padding, and sigmoid activation

        x = input = tf.keras.layers.Input([args.z_dim])
        x = dense_batchnorm_relu(1024)(x)
        x = dense_batchnorm_relu(MNIST.H // 4 * MNIST.W // 4 * 64)(x)
        x = tf.keras.layers.Reshape([MNIST.H // 4, MNIST.W // 4, 64])(x)
        x = batchnorm_relu(tf.keras.layers.Conv2DTranspose(32, 5, 2, 'same', use_bias=False))(x)
        output = tf.keras.layers.Conv2DTranspose(1, 5, 2, 'same', activation=tf.nn.sigmoid)(x)
        ...
        x = input = tf.keras.layers.Input([MNIST.H, MNIST.W, MNIST.C])
        x = batchnorm_relu(tf.keras.layers.Conv2D(32, 5, padding='same', use_bias=False))(x)
        x = tf.keras.layers.MaxPool2D(pool_size=2, strides=2)(x)
        x = batchnorm_relu(tf.keras.layers.Conv2D(64, 5, padding='same', use_bias=False))(x)
        x = tf.keras.layers.MaxPool2D(pool_size=2, strides=2)(x)
