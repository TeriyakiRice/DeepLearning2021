...
class Network(tf.keras.Model):
    def __init__(self, args):
        ...
        rnn_layer_dict = {'LSTM': tf.keras.layers.LSTM(units=args.rnn_cell_dim, return_sequences=True),
                          'GRU': tf.keras.layers.GRU(units=args.rnn_cell_dim, return_sequences=True),
                          'SimpleRNN': tf.keras.layers.SimpleRNN(units=args.rnn_cell_dim, return_sequences=True)}
        rnn_layer = rnn_layer_dict[args.rnn_cell]
        x = rnn_layer(sequences)
        ...
        if not (args.hidden_layer == 0):
            x = tf.keras.layers.Dense(activation=tf.keras.activations.relu, units=args.hidden_layer)(x)

        # TODO: Generate `predictions` using a fully connected layer
        # with one output and `tf.nn.sigmoid` activation.
        predictions = tf.keras.layers.Dense(activation=tf.keras.activations.sigmoid, units=1)(x)
